//! HTML entity / character-reference decoder.
//!
//! Shared by inline scanning (character references in general text), fenced-code info strings, and
//! link-destination/title normalisation (CommonMark says an entity reference in any of these "counts
//! as its value"). Three forms are recognised, each requiring a terminating `;`:
//!
//!   * **named** — `&name;` against the full WHATWG HTML5 named-character-reference set (the exact set
//!     CommonMark mandates: 2125 names, generated from the HTML5 spec; see [`NAMED`]),
//!   * **decimal** — `&#DDD;`, and
//!   * **hex** — `&#xHHH;` / `&#XHHH;`.
//!
//! A `NUL`, an out-of-range code point, or a surrogate decodes to U+FFFD (`�`) per the spec. An
//! unknown name or a malformed reference yields `None`, leaving the caller to keep the text literal.

/// If `s` begins with a valid HTML character reference, return its decoded text and the byte length
/// of the reference (including the leading `&` and trailing `;`). Otherwise return `None`.
pub fn decode_entity(s: &str) -> Option<(String, usize)> {
    let b = s.as_bytes();
    if b.first() != Some(&b'&') {
        return None;
    }
    // Numeric: &#123; (decimal) or &#x1F; / &#X1F; (hex). The HTML5 spec caps numeric references at
    // 7 digits (decimal) / 6 hex digits; longer runs are not valid references.
    if b.get(1) == Some(&b'#') {
        let (radix, start, max_digits) = match b.get(2) {
            Some(b'x') | Some(b'X') => (16, 3, 6),
            _ => (10, 2, 7),
        };
        let mut j = start;
        while j < b.len() && b[j] != b';' && (b[j] as char).is_digit(radix) {
            j += 1;
        }
        if j >= b.len() || b[j] != b';' || j == start || j - start > max_digits {
            return None;
        }
        let code = u32::from_str_radix(&s[start..j], radix).ok()?;
        // NUL, surrogates, and out-of-range scalar values all render as U+FFFD per the spec.
    ...
    // Named: &name; — the name is ASCII alphanumeric and must be terminated by `;`.
    let mut j = 1;
    ...
    named_entity(&s[1..j]).map(|v| (v.to_string(), j + 1))
}

/// Look up a named entity (without the surrounding `&`/`;`) in the HTML5 table by binary search.
/// Returns the replacement text, or `None` if the name is not a defined reference.
...
/// The full WHATWG HTML5 named character reference table (the names that require a trailing `;`),
/// sorted by name for binary search. Generated from the HTML5 specification's entity list; this is
/// exactly the set CommonMark's "Entity and numeric character references" rule decodes.
#[rustfmt::skip]
...
mod tests {
    ...
    fn named_table_is_sorted() {
        // Binary search requires the table be sorted by name.
        assert!(NAMED.windows(2).all(|w| w[0].0 < w[1].0));
    }

    #[test]
    fn decodes_named() {
        assert_eq!(decode_entity("&amp;rest").unwrap(), ("&".to_string(), 5));
        assert_eq!(decode_entity("&copy;").unwrap(), ("\u{a9}".to_string(), 6));
        assert_eq!(decode_entity("&ngE;").unwrap().0, "\u{2267}\u{338}");
        assert_eq!(decode_entity("&nbsp;").unwrap().0, "\u{a0}");
    }

    #[test]
    fn decodes_numeric() {
        assert_eq!(decode_entity("&#35;").unwrap(), ("#".to_string(), 5));
        assert_eq!(decode_entity("&#X22;").unwrap(), ("\"".to_string(), 6));
        assert_eq!(decode_entity("&#xcab;").unwrap().0, "\u{cab}");
        // NUL and out-of-range → U+FFFD.
        assert_eq!(decode_entity("&#0;").unwrap().0, "\u{FFFD}");
        assert_eq!(decode_entity("&#x10FFFF;").unwrap().0, "\u{10FFFF}");
        assert_eq!(decode_entity("&#x110000;").unwrap().0, "\u{FFFD}");
    }

    #[test]
    fn rejects_unknown_and_malformed() {
        assert!(decode_entity("&MadeUpEntity;").is_none());
        assert!(decode_entity("&amp").is_none()); // no terminating ';'
        assert!(decode_entity("&#;").is_none());
        assert!(decode_entity("&#x;").is_none());
        assert!(decode_entity("&;").is_none());
        // Overlong numeric runs are not valid references.
        assert!(decode_entity("&#12345678;").is_none());
    }
}
